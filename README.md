...
当前代码已将操作入口收敛为 **F01 研究基础层 + M01–M02 两个工作台**：
...
运行 `npm test` 后，执行 `npm run build` 进行生产构建验证。
...
- **F01 — 研究基础层**：统一管理 K 线、多周期数据、永续衍生品上下文、图形对象、人工标签和研究记录。
- **M01 — 市场工作台**：主图、周期和日期定位、十字光标、快速人工判断、永续衍生品上下文。
- **M02 — 结构研究工作台**：趋势线、水平位、磁吸、样式、智能校准、结构案例、买点研究区间、低周期扫描、人工判断和案例历史。

交互原则：主图始终是中心；高频操作尽量直接在图上完成；所有能自动保存的研究状态默认自动保存；同一趋势线和水平位在不同周期中是完全相同的结构对象，周期只改变 K 线观察分辨率。
